...
describe('analyseEpic', () => {
  ...
  it('throws LlmAnalysisFailedError immediately when epicKey does not match snapshot', async () => {
    const wrongKey = { ...analysisResultValid, epicKey: 'CM-9999' };
    const mockClient = {
      createMessage: vi.fn().mockResolvedValue(makeToolUseResponse(wrongKey)),
    } as unknown as LlmClient;

    await expect(analyseEpic(mockClient, snapshotAtRisk as EpicSnapshot, config)).rejects.toThrow(
      LlmAnalysisFailedError,
    );
    // No retry on wrong epicKey — throws immediately after first attempt
    expect(mockClient.createMessage).toHaveBeenCalledTimes(1);
  });

  it('retries once when Zod validation fails on first attempt then succeeds', async () => {
    // Invalid input: missing required fields
    const invalidInput = { epicKey: 'CM-1234', weeklyUpdateFound: 'not-a-boolean' };
    const mockClient = {
      createMessage: vi
        .fn()
        .mockResolvedValueOnce(makeToolUseResponse(invalidInput))
        .mockResolvedValue(makeToolUseResponse(analysisResultValid)),
    } as unknown as LlmClient;
  ...
  it('throws LlmAnalysisFailedError after exhausting retries on Zod validation failure', async () => {
    const invalidInput = { epicKey: 'CM-1234' }; // missing required fields
    const mockClient = {
      createMessage: vi.fn().mockResolvedValue(makeToolUseResponse(invalidInput)),
    } as unknown as LlmClient;
  ...
  it('passes the configured model and maxTokens to createMessage', async () => {
    const mockClient = {
      createMessage: vi.fn().mockResolvedValue(makeToolUseResponse(analysisResultValid)),
    } as unknown as LlmClient;

    await analyseEpic(mockClient, snapshotAtRisk as EpicSnapshot, config);
  ...
  it('forces tool_choice to submit_epic_analysis', async () => {
    const mockClient = {
      createMessage: vi.fn().mockResolvedValue(makeToolUseResponse(analysisResultValid)),
    } as unknown as LlmClient;
