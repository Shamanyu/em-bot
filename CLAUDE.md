...
A weekly automated analysis bot for engineering managers at Springboard. It runs every Tuesday, reads in-scope Epics from a JIRA saved filter, generates structured per-Epic analysis using Claude (tool-use), and posts ADF-formatted comments directly to JIRA. A team rollup comment is also posted to a designated ticket.
...
- `JIRA_BASE_URL` — e.g. `https://springboard-edu.atlassian.net`
...
- `jira.filterId: 13297` — "EM Bot EPICs" saved filter owned by Shubham Shamanyu
- `jira.rollupTicketKey: CM-1571` — team rollup destination ticket
...
- Don't close/resolve CM-1571 — it accumulates weekly rollup comments
